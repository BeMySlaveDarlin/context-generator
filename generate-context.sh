...
if [ -n "$SRC_PATH" ] && [ -d "$SRC_PATH" ]; then
    ...
    # Structure
    BACKEND_STRUCTURE="$BACKEND_DIR/structure.txt"
    EXCLUDE_PATTERNS="node_modules|cache"

    if [ -n "$FRONTEND_PATH" ] && [[ "$FRONTEND_PATH" == "$SRC_PATH"* ]]; then
        FRONTEND_REL="${FRONTEND_PATH#$SRC_PATH/}"
        EXCLUDE_PATTERNS="$EXCLUDE_PATTERNS|${FRONTEND_REL%%/*}"
    fi

    if [ -n "$MIGRATIONS_PATH" ] && [[ "$MIGRATIONS_PATH" == "$SRC_PATH"* ]]; then
        MIGRATIONS_REL="${MIGRATIONS_PATH#$SRC_PATH/}"
        EXCLUDE_PATTERNS="$EXCLUDE_PATTERNS|${MIGRATIONS_REL%%/*}"
    fi

    tree -I "$EXCLUDE_PATTERNS" "$SRC_PATH" 2>/dev/null | iconv -f UTF-8 -t ASCII//TRANSLIT > "$BACKEND_STRUCTURE" || \
    find "$SRC_PATH" -type d | grep -vE "$(echo $EXCLUDE_PATTERNS | tr '|' '\n' | paste -sd'|')" > "$BACKEND_STRUCTURE"

    FILE_COUNT=$(find "$SRC_PATH" -type f -name "*.php" 2>/dev/null | wc -l)
    echo -e "\nTotal PHP files: $FILE_COUNT" >> "$BACKEND_STRUCTURE"
...
if [ -n "$FRONTEND_PATH" ] && [ -d "$FRONTEND_PATH" ]; then
    ...
    # Structure
    FRONTEND_STRUCTURE="$FRONTEND_DIR/structure.txt"
    tree -L 3 -I 'node_modules|dist|build|.next|.nuxt' "$FRONTEND_PATH" 2>/dev/null | iconv -f UTF-8 -t ASCII//TRANSLIT > "$FRONTEND_STRUCTURE" || \
    find "$FRONTEND_PATH" -maxdepth 3 -type d | grep -v 'node_modules\|dist\|build' > "$FRONTEND_STRUCTURE"

    JS_COUNT=$(find "$FRONTEND_PATH" -type f \( -name "*.js" -o -name "*.jsx" -o -name "*.ts" -o -name "*.tsx" -o -name "*.vue" \) 2>/dev/null | wc -l)
    echo -e "\nTotal JS/TS/Vue files: $JS_COUNT" >> "$FRONTEND_STRUCTURE"
